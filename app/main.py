...
def create_parameter_or_formulavalue(row_dict, filepath, tree, node_info, stats):
    ...
    if last_part.startswith("Parameter["):
        ...
        # 2) ERPAlias (always present, even if blank)
        erp = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}ERPAlias")
        erp_val = row_dict.get("ERPAlias", "").strip()
        if erp_val:
            erp.text = erp_val

        # 3) PLCReference
        plc = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}PLCReference")
        plc_val = row_dict.get("PLCReference", "").strip()
        plc.text = plc_val or None

        # Detect type
        has_string = bool(row_dict.get("String", "").strip())
        has_integer = bool(row_dict.get("Integer", "").strip())
        has_real = bool(row_dict.get("Real", "").strip())
        has_enum = bool(row_dict.get("EnumerationSet", "").strip())

        # 4) Then type-specific order
        if has_string:
            # string value
            s = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}String")
            s.text = row_dict.get("String", "")
            eu = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}EngineeringUnits")
            eu_val = row_dict.get("EngineeringUnits", "").strip()
            if eu_val:
                eu.text = eu_val

        elif has_integer:
            # integer value
            i = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Integer")
            i.text = row_dict.get("Integer", "")
            hi = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}High")
            hi.text = row_dict.get("High", "")
            lo = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Low")
            lo.text = row_dict.get("Low", "")
            eu = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}EngineeringUnits")
            scale = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Scale")
            eu_val = row_dict.get("EngineeringUnits", "").strip()
            if eu_val:
                eu.text = eu_val
            scale_val = row_dict.get("Scale", "").strip()
            if scale_val:
                scale.text = scale_val

        elif has_real:
            # real value
            r = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Real")
            r.text = row_dict.get("Real", "")
            hi = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}High")
            hi.text = row_dict.get("High", "")
            lo = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Low")
            lo.text = row_dict.get("Low", "")
            ...
            # enumeration value
            es = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}EnumerationSet")
            es.text = row_dict.get("EnumerationSet", "")
            em = etree.SubElement(
                new_el, "{urn:Rockwell/MasterRecipe}EnumerationMember"
            )
            em.text = row_dict.get("EnumerationMember", "")

        # Insert parameter after last one
        last_param = None
        ...
        if last_param is not None:
            root.insert(root.index(last_param) + 1, new_el)
        else:
            root.append(new_el)
    ...
    # --- FORMULAVALUE BRANCH ---
        ...
        # 1) Name, Display, Defer
        nm = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Name")
        nm.text = row_dict.get("Name", "")
        dp = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Display")
        dp_val = row_dict.get("Display", "").strip()
        if dp_val:
            dp.text = dp_val
        df = etree.SubElement(new_el, "{urn:Rockwell/MasterRecipe}Defer")
        df_val = row_dict.get("Defer", "").strip()
        if df_val:
            df.text = df_val
        ...
        # 4) FormulaValueLimit (if any)
        limit_keys = [k for k in row_dict if k.startswith("FormulaValueLimit_")]
        if any(row_dict.get(k, "").strip() for k in limit_keys):
            fvl = etree.SubElement(
                new_el, "{urn:Rockwell/MasterRecipe}FormulaValueLimit"
            )
            ver = row_dict.get("FormulaValueLimit_Verification", "").strip()
            if ver:
                fvl.set("Verification", ver)
            for subtag in [
                "LowLowLowValue",
                "LowLowValue",
                "LowValue",
                ...
                sv = row_dict.get(f"FormulaValueLimit_{subtag}", "").strip()
                if sv:
                    child = etree.SubElement(
                        fvl, f"{{urn:Rockwell/MasterRecipe}}{subtag}"
                    )
                    child.text = sv

        # Append under its <Step>
